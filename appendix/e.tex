...
Assuming that at any time $t$ each point in the grid cell is either at $C_{m}$ or at a constant moisture state $c$ (\cite{ren1992xinanjiang}), the mean areal water storage $S$ associated with soil water-holding capacity $c$ at time $t$ is given by equation~\ref{eq:maws}
...
A linear reservoir is assumed to model base flow $RB$. The storage of the linear reservoir is linearly related to output, namely, base flow by a storage constant $\beta$ (\cite{chow1988applied}, Equation~\ref{eq:rbflow}). 
...
\begin{conditions}
STRGV       & Reservoir storage at the end of month $m$ \\
STRGPV      & Initial or Beginning-of-period storage in the First Month \\
...
\label{eq:resevap}
\begin{aligned}
EVAPV_{fpu,m} &= - EVAPSLACKV_{fpu,m} \\
              &\quad + PRET1_{fpu, m} \\
              &\quad \times \frac{1}{2} \left[ 
                  \begin{aligned}
                      & STRGV_{fpu,m} \\
                      &+ ISTRG1_{fpu} \mid m = 1 \wedge t \neq 1 \\
                      &+ STRGV_{fpu,m \ominus 1} \mid m = 1 \wedge t = 1 \\
                      &+ STRGV_{fpu, m-1} \mid m \neq 1
                  \end{aligned}
              \right] \\
              &\quad \times STRGC1_{fpu}^{-1}
\end{aligned}
\end{equation}
where
\begin{conditions}
EVAPV       & Monthly reservoir evaporation \\
EVAPSLACKV  & Slack variable for reservoir evapotranspiration \\
PRET1       & Evaporation when reservoir is full \\
STRGV       & Reservoir storage at the end of month $m$ \\
ISTRG1      & Initial storage (in $m$=1 i.e., january equals $m$=12 i.e., December) \\
STRGC1      & basin storage capacity \\
\end{conditions}
...
The total surface water withdrawal in a given year cannot exceed surface water withdrawal capacity i.e., $\sum_{m} SWDPV_{fpu, m} \le SWDCAP_{fpu}$. The capacity has been converted into a consumptive use term, using estimated depletion coefficients of domestic, industrial, and agricultural sectors. Water withdrawal data by source around year 2005 from FAO’s AQUASTAT global databasewere used to estimate surface and groundwater withdrawal capacity values (\cite{FAO2014}), considering inter-annual variability of water demand at the FPU level.
...
\label{eq:cofr}
\begin{aligned}
SPV_{fpu,m} \ge \frac{1}{24} \times CFY_{fpu} \times \sum_{m} \Big[ &IRW_{fpu, m} \\
&- GSEXG_{fpu} \times GRCHG{fpu,m2} \\
&+ INFLOWV_{fpu,m2} - EVAPV_{fpu,m2} \Big]
\end{aligned}
\end{equation}
wehre
\begin{conditions}
...
IRW & Monthly runoff \\
GSEXG & Percentage of groundwater recharge from surface water \\
GRCHG & Groundwater recharge \\
INFLOWV     & Monthly inflows from upstream food production units \\
...
Equations \ref{eq:irr_yield_1} and \ref{eq:irr_yield_2} provide alternative approaches to aggregating the monthly yield shocks to produce the annual shock. Both approaches are based on \cite{rao1988simple}. Equation \ref{eq:irr_yield_1} shows a multiplicative representation of monthly yield shocks, where the annual yield reduction shock equals the product during the months of 1 minus the monthly irrigated yield reductions. It is a nonlinear equation that requires that the model be solved with a nonlinear programming (NLP) solver. Equation \ref{eq:irr_yield_2} shows an additive representation of monthly yield shocks where the annual yield reduction shock equals 1 minus the sum of the monthly irrigated yield reduction terms. The equation is linear, and the model can be solved as a quadratic programming (QCP) problem. 

\begin{subequations}
\label{eq:yield_reduction}
\begin{align}
    \label{eq:irr_yield_1}
    IYRYV_{fpu,j} &= \prod_{m \mid Ky_{fpu,j,m}} (1 - IYRMV_{fpu, j, m}) \\[15pt]
    \label{eq:irr_yield_2}
    IYRYV_{fpu, j} &= 
    \begin{cases} 
        1 - \frac{\sum_{m} \left( KyMod_{j} \times Ky_{fpu,j,m} \times IYRMV_{fpu,j,m} \right)}{\sum_{m} \left( KyMod_{j} \times Ky_{fpu,j,m} \right)} & \text{if } \sum_{m} Ky_{fpu,j,m} > 0 \\
        0 & \text{otherwise}
    \end{cases} \\[15pt]
    \label{eq:irr_yield_3}
    IYRYV_{fpu, j} &= \prod_{m \mid Ky_{fpu,j,m}} (1 - IYRMV_{fpu, j, m})^{wKy_{fpu, j, m}}
\end{align}
\end{subequations}

where
\begin{conditions}
IYRYV & Annual irrigated yield multiplier \\
IYRMV & Monthly irrigated yield reduction rate \\
Ky  & Monthly crop yield coefficient \\
KyMod & Technology-Specific Yield Sensitivity Mapping \\
\end{conditions}

As $IYRMV$ is the rate of reduction in yield by month, so a value of 1 means total crop loss. $IYRYV$ is the annual yield multiplier, so the new yield equals old yield times IYRY, and 1 means no crop loss (see equation \ref{eq:icwasmobjective}).
...
The objective function of the ICWASM module includes total revenue and a risk-aversion term to dampen changes in allocation of water to crops from the previous year and is represented mathematically by equation~\ref{eq:icwasmOBJ}. The first term allocates available water to crops to maximize the expected value of output, given water shortages and yield shocks. The second term minimizes the slack variables. Slack variable $IYRMSLKV$ is chosen to ensure $IYRMV$ $\leq$ 1.

\begin{equation}
\label{eq:icwasmobjective}
\begin{aligned}
OBJV2 = &\sum_{fpu, j} \left( 1 - IYRYV_{fpu,j} \right)^2 \times YldIrr_{fpu,j} \times PP_{fpu,j} \\
        &+ 10^{4} \times \sum_{j, fpu, m} IYRMSLKV_{fpu,j,m}
\end{aligned}
\end{equation}

where
\begin{conditions}
YldIrr & Irrigated yield \\
PP & Producer prices at the FPU level for the previous year \\
IYRMSLKV & Slack variable to ensure $IYRMV$ $\le$ 1\\
\end{conditions}