...
Livestock water demand is estimated using livestock numbers and water consumptive use per unit of livestock ($w_{lv}$), including beef, milk, pork, poultry, eggs, and sheep and goats (\textcolor{red}{de Fraiture 2007; Steinfeld et al. 2006}). The total number of live animals during a year includes slaughtered animals, the followers herd, and other categories (for example, milk-producing animals). The total number of animals is calculated based on the number of slaughtered animals ($QS_{lv}$) and a ratio of the number of slaughtered animals to the total number of animals ($r_{lv}$) (eq.~\ref{eq:lwd}).
...
Irrigation water demand is assessed as the portion of crop water requirement (eq.~\ref{eq:niwd}) not satisfied by precipitation or soil moisture based on hydrologic and agronomic characteristics. Net irrigation water demand ($NIWD$) in an FPU is calculated based on an empirical crop water requirement function (\textcolor{red}{Doorenbos and Pruit 1977}) and irrigated area of the crop.
...
Effective rainfall depends on total rainfall ($PT$), previous soil moisture content ($SM0$), maximum crop evapotranspiration ($ETM$), and soil characteristics (hydraulic conductivity $K$, moisture content at field capacity $Zs$, and others). $PE$ is calculated by a SCS (Soil Conservation Service) method (\textcolor{red}{USDA-SCS 1993}), given $PT$, $ETM$, and effective soil water storage.
...
in which, $SF$ is the soil water storage factor and is calculated with
    ...
    \end{align}
\end{subequations}
where $D$ represents the usable soil water storage in inches and is generally calculated as 40 to 60 percent of available soil water capacity in the crop root zone, depending on the irrigation management practices in use (\textcolor{red}{USDA-SCS 1993}).
...
The concept of basin efficiency was discussed and various definitions were provided by \textcolor{red}{Keller, Keller and Seckler (1996)}. Basin efficiency is defined as the ratio of beneficial water depletion (crop evapotranspiration and salt leaching) to total irrigation water depletion at the FPU scale. Basin efficiency in the base year is calculated as the ratio of the net irrigation water demand ($NIWD$, equation~\ref{eq:niwd}) to the total irrigation water depletion estimated from records (\textcolor{red}{Shiklomanov 1999}). Basin efficiency in future years is assumed to increase at a prescribed rate in an FPU depending on water infrastructure investment and water management improvement in the FPU.
