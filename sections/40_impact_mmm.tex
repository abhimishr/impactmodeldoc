...
\begin{equation}
    \label{eq:area}
    \begin{aligned}
    Area_{j, fpu, lnd} = AreaInt_{j, fpu, lnd} \times AreaInt2_{j, fpu, lnd} \\
    \times (WF_{fpu, lnd})^{LDE_{j, fpu, lnd}} \times \left ( \frac{MRP_{j, fpu, lnd}}{MRP0_{j, fpu, lnd}} \right )^{A_{j, fpu, lnd}}
    \end{aligned}
\end{equation}
where:
\begin{conditions}
...
\begin{equation}
    \label{eq:yields}
    \begin{aligned}
    Yield_{j, fpu, lnd} = YieldInt_{j, fpu, lnd} \times YieldInt2_{j, fpu, lnd} \times YieldIntB2_{fpu}\\
    \times YldShk_{j, fpu, lnd} \times YldCliShk_{j, fpu, lnd} \\
    \times \prod_{pfac} (WF_{fpu, pfac})^{YE_{j, fpu, lnd,pfac}} \\
    \times \left ( \frac{PNET_{j, cty}}{PNET0_{j, cty}} \right )^{Y_{j, fpu, lnd}} \forall cty \in fpu \\
    \end{aligned}
\end{equation}
where:
\begin{conditions}
...
\begin{equation}
    \label{eq:animals}
    \begin{aligned}
        AnN_{j, fpu, lvs} = AnNInt_{j, fpu, lvs} \times AnNInt2_{j, fpu, lvs} \\
        \times \prod_{jl}\left ( \frac{PNET_{jl,cty}}{PNET0_{jl,cty}} \right )^{AN_{\varepsilon}} \times \prod_{cfeed}\left ( \frac{PC_{cfeed, cty}}{PC0_{cfeed, cty}} \right )^{Feed_{\varepsilon}} \forall cty \in fpu
    \end{aligned}
\end{equation}
where:
\begin{conditions}
...
\begin{equation}
    \label{eq:qd}
    QD_{c, cty} = \big[ \big(\sum_{h} QH_{c, h, cty}\big) \times (1+WasteH_{c, cty}) \big] + QInterm_{c, cty} + QL_{c, cty} + QBF_{c, cty} + QOth_{c, cty}
\end{equation}
where:
\begin{conditions}
...
\begin{equation}
    \label{eq:qh}
    \begin{aligned}
        QH_{c, h, cty} = QHDint_{c, h, cty} \times QHDInt2_{c, h, cty} \\
        \times \left(\frac{pcGDP_{h, cty}}{pcGDP0_{h, cty}}\right)^{Ie_{c, h, cty}} \times \left(\frac{popH_{h, cty}}{popH0_{h, cty}}\right) \\
        \times \prod_{cc} \left(\frac{PC_{c, cty} \times (1 - CSE_{c, cty})}{PC0_{c, cty} \times (1 - CSE0_{c, cty})}\right)^{Fe_{c, cc, h, cty}}
    \end{aligned}
\end{equation}
where: 
\begin{conditions}
...
\begin{equation}
    \label{eq:qoth1}
    \begin{aligned}
        QOth_{c, cty} = QOthInt_{c, cty} \times QOthInt2_{c, cty} \times  \left(\frac{pcGDP_{cty}}{pcGDP0_{cty}}\right)^{OIe_{c, cty}} \times \\ \left(\frac{pop_{cty}}{pop0_{cty}}\right) \times \prod_{cc=c} \left(\frac{PC_{c, cty}}{PC0_{c, cty}}\right)^{OPe_{c, cc, cty}}
    \end{aligned}
\end{equation}
where: 
\begin{conditions}
